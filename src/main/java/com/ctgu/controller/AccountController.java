...
public class AccountController {
    ...
    public String myExam(HttpServletRequest request, @RequestParam(value = "page", defaultValue = "1") int page, Model model) {
        ...
        Map<String, Object> data = gradeService.getGradesByStudentId(page, Const.gradePageSize, currentAccount.getId());
    ...
    public String myDiscussPost(HttpServletRequest request, @RequestParam(value = "page", defaultValue = "1") int page, Model model) {
        ...
        Map<String, Object> data = postService.getPostsByAuthorId(page, Const.postPageSize, currentAccount.getId());
        model.addAttribute(Const.DATA, data);
        model.addAttribute(Const.CURRENT_ACCOUNT, currentAccount);
    ...
    public AjaxResult updatePassword(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            String md5OldPassword = MD5.md5(Const.MD5_SALT+oldPassword);
            String md5NewPassword = MD5.md5(Const.MD5_SALT+newPassword);
            if (StringUtils.isNotEmpty(newPassword) && StringUtils.isNotEmpty(confirmNewPassword)
                    && !newPassword.equals(confirmNewPassword)) {
                return AjaxResult.fixedError(WebError.NOT_EQUALS_CONFIRM_PASSWORD);
            }
            Account currentAccount = (Account) request.getSession().getAttribute(Const.CURRENT_ACCOUNT);
...
//                String pwd = MD5.md5(Const.MD5_SALT+password);
    ...
    public String logout(HttpServletRequest request) {
        request.getSession().setAttribute(Const.CURRENT_ACCOUNT,null);
        LOG.info("logout");
        return "redirect:/";
    }
    ...
    public Map<String,Object> uploadAvatar(HttpServletRequest request, @RequestParam("file") MultipartFile file) throws IllegalStateException, IOException{
        ...
        try {
            ...
            //存储图片的物理路径
            String file_path = Const.UPLOAD_FILE_IMAGE_PATH;
            ...
            if(file!=null && oldFileName!=null && oldFileName.length()>0){
                ...
                return AjaxResult.fixedError(WebError.UPLOAD_FILE_IMAGE_NOT_QUALIFIED);
            }
        } catch (Exception e) {
            LOG.error(e.getMessage(), e);
            return AjaxResult.fixedError(WebError.UPLOAD_FILE_IMAGE_ANALYZE_ERROR);
        }
        return ajaxResult;
    }
    ...
    public AjaxResult addAccount(@RequestBody Account account) {
        ...
        if(existAccount == null) {//检测该用户是否已经注册
            account.setPassword(MD5.md5(Const.MD5_SALT+account.getPassword()));
            account.setAvatarImgUrl(Const.DEFAULT_AVATAR_IMG_URL);
            account.setDescription("");
            int accountId = accountService.addAccount(account);
            return new AjaxResult().setData(accountId);
        }
        return AjaxResult.fixedError(WebError.AREADY_EXIST_USERNAME);
    }
    ...
    public AjaxResult updateAccount(@RequestBody Account account) {
        //TODO::只修改部分信息时，剩余信息要保持原有的值，不能置空
        AjaxResult ajaxResult = new AjaxResult();
