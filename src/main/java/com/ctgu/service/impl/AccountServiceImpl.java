...
public class AccountServiceImpl implements AccountService {
    ...
    @Override
    public Map<String, Object> getAccountsByLevel(int pageNum, int pageSize, int level) {
        Map<String, Object> data = new HashMap<>();
        int count = accountMapper.getCountByLevel(level);
        if (count == 0) {
            data.put("pageNum", 0);
            data.put("pageSize", 0);
            data.put("totalPageNum", 1);
        ...
        PageHelper.startPage(pageNum, pageSize);
        List<Account> accounts = accountMapper.getAccountsByLevel(level);
        data.put("pageNum", pageNum);
        data.put("pageSize", pageSize);
        data.put("totalPageNum", totalPageNum);
        data.put("totalPageSize", count);
    ...
    @Override
    public List<Account> getAccountsByLevel(int teacherLevel) {
        return accountMapper.getAccountsByLevel(teacherLevel);
    }
}
