...
import com.ctgu.util.word2html.bean.WordHtmlDto;
...
public class Word2Html implements Doc2Html {
	...
	private WordHtmlDto docToHtml(String filePath) throws Exception {
		...
		String fileDir = System.currentTimeMillis() + "";
//		String outDir = Config.getDir(System.currentTimeMillis() + "");
		String outDir = filePath + fileDir + "/";
		...
		resultDto.setFilePath(fileDir + "/content.html");
		return resultDto;
	}
...
//			String outDir = Config.getDir(System.currentTimeMillis() + "");
			String fileDir = System.currentTimeMillis() + "";
			String outDir = Const.UPLOAD_FILE_HTML_PATH + fileDir + "/";
			File dirFile = new File(outDir);
			if (!dirFile.exists()) {
				dirFile.mkdirs();
			}
